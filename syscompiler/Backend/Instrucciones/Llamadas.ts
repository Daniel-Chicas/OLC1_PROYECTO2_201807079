...
export class Llamadas extends Instruccion{
    constructor(private id: string, private parametros: Expresion[], linea: number, columna: number){
        super(linea, columna)
    }
    ...
    public execute(entorno: Entorno, simbolos: TablaSimbolos){
        ...
        if(metodo == null){
            throw new Error_(this.line, this.column, "Semántico", "No se encontró el método: "+this.id)
        }else{
            var parametros = []
            ...
            if(parametros.length != this.parametros.length){
                throw new Error_(this.line, this.column, "Semántico", "No coincide la cantidad de parámetros de la llamada con el método.")
            }else{
                for (let i = 0; i < this.parametros.length; i++) {
                    ...
                    if(!aprobado){
                        throw new Error_(this.line, this.column, "Semántico", "Los tipos de datos de la llamada al método no coinciden con los esperados.")
                    }
                }
                var aux = entorno;
                var env;
                ...
                while(entorno != null){
                    if(entorno.anterior == null) env = entorno
                    entorno = entorno.anterior 
                }
                entorno = aux;

                var nuevoEntorno = new Entorno(env, metodo.id)
                for (let i = 0; i < this.parametros.length; i++) {
                    var tipoId = parametros[i].split("-")
                ...
                if(retorna != null || retorna != undefined){
                    throw new Error_(this.line, this.column, "Semántico", "Los métodos no retornan ningún valor.")
                }
            }
        }
    }
...
export class LlamadasFunciones extends Expresion{
    constructor(private id: string, private parametros: Expresion[], linea: number, columna: number){
        super(linea, columna)
    }
    ...
    public execute(entorno: Entorno, simbolos: TablaSimbolos){
        ...
        if(funcion == null){
            throw new Error_(this.line, this.column, "Semántico", "No se encontró la función: "+this.id)
        }else{
            var parametros = []
            ...
            if(parametros.length != this.parametros.length){
                throw new Error_(this.line, this.column, "Semántico", "No coincide la cantidad de parámetros de la llamada con la función.")
            }else{
                for (let i = 0; i < this.parametros.length; i++) {
                    var aprobado = false;
                    var tipoId = parametros[i].split("-")
                    ...
                    if(!aprobado){
                        throw new Error_(this.line, this.column, "Semántico", "Los tipos de datos de la llamada la función no coinciden con los esperados.")
                    }
                }
                var aux = entorno;
                var env;
                ...
                var nuevoEntorno = new Entorno(env, funcion.id)
                for (let i = 0; i < this.parametros.length; i++) {
                    var tipoId = parametros[i].split("-")
                    const value = this.parametros[i].execute(entorno, simbolos)
                    var tipo = -1;
                ...
                if(retorna != null || retorna != undefined){
                    if(retorna == null){
                        throw new Error_(this.line, this.column, "Semántico", "No se permite un retorno de tipo null.")
                    }else{
                        var tipo = -1;
